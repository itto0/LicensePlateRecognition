...
def loadKNNDataAndTrainKNN():
    ...
    try:
        npaClassifications = np.loadtxt("helper/Classifications.txt", np.float32)
    except:
        print("error, failed load file classifications.txt\n")
        return False

    try:
        npaFlattenedImages = np.loadtxt("helper/Flattened_Images.txt", np.float32)
    except:
...
def findPossibleCharsInPlate(imgGrayscale, imgThresh):
    ...
    for contour in contours:
        possibleChar = PossibleCharacter.PossibleCharacter(contour)
